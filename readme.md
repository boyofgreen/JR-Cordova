...
### Offline Feature
The plugin implements an optional offline feature that will show an offline page whenever network connectivity is lost.

The feature can be enabled with the following property in the manifest.json file.

```
{
  ...
  "mjs_offline_feature": true
  ...
}
```

By default, the page shows a suitable message alerting the user about the loss of connectivity. To customize the offline experience, a page named **offline.html** can be placed in the **www** folder of the application and it will be used instead.
